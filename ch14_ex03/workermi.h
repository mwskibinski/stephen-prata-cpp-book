...
protected:
	enum {
		inny, alt, kontralt, sopran, bas, baryton, tenor
	};
	enum {Vtypes = 7};
	void Data() const;
	void Get();
...
public:
	SingingWaiter() { }
	SingingWaiter(const std::string & s, long n, int p = 0, int v = inny)
		: Worker(s, n), Waiter(s, n, p), Singer(s, n, v) { }
	SingingWaiter(const Worker & wk, int p = 0, int v = inny)
		: Worker(wk), Waiter(wk, p), Singer(wk, v) { }
	SingingWaiter(const Waiter & wt, int v = inny)
